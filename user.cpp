...
int main() {
  ...
  camera.position = Vector3Add(
      Vector3Scale(Vector3Subtract(camera.position, camera.target), 4),
      camera.position);
  ...
  Texture2D tex =
      LoadTexture("/home/kpgm/code/ecsDodLearning++/Tiny Swords (Free "
                  "Pack)/Units/Black Units/Archer/Archer_Idle.png");
  ...
  for (int i = 0; i < 20000; i++) {
    ...
    // tc.translate = {0.0f,0.0f,0.0f};
    auto &r = core.add_component<RigidBodyComponent>(e);
    r.velocity.y = dis(gen) * 5;
  }
